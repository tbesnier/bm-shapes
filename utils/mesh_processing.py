...
def create_gif_shape(list_data, faces, file_dir, file_name, x_view=20, y_view=20, z_view=-10, auto_scale = True):
    filenames = []
    listpq = list_data
    steps = []   
    faces = faces
    ...
    for i in range(len(listpq)):
        ...
        if auto_scale:
            ...
            V = (V-(V.max(0)+V.min(0))/2)/scale
        MVP = perspective(25,1,1,100) @ translate(0,0,-3.5) @ xrotate(x_view) @ yrotate(y_view) @ zrotate(z_view)
        V = np.c_[V, np.ones(len(V))]  @ MVP.T
        V /= V[:,3].reshape(-1,1)
        V = V[F]
        T =  V[:,:,:2]
