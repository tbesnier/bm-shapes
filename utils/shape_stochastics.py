...
def compute_diffusion_process(x, y, z, theta, phi, Q, b, sigma, n = 25, t=1, n_step = 10, make_gif = True, file_dir = "tests", file_name = "test.gif"):
    
    """Compute Q-Diffusion process on the spherical harmonic basis of a shape defined by (x,y,z) triplet of functions from the sphere
    ...
    reconstr_stoch_x = basis.sph_harm_reconstruct_sde(coeffs_x, Q, t, theta = theta, phi = phi, b = b, sigma = sigma, n_step = n_step)[0]
    reconstr_stoch_y = basis.sph_harm_reconstruct_sde(coeffs_y, Q, t, theta = theta, phi = phi, b = b, sigma = sigma, n_step = n_step)[0]
    reconstr_stoch_z = basis.sph_harm_reconstruct_sde(coeffs_z, Q, t, theta = theta, phi = phi, b = b, sigma = sigma, n_step = n_step)[0]
...
def compute_diffusion_process_shape(x_mesh, y_mesh, z_mesh, faces, Q, b, sigma, n = 25, t=1, n_step=10, make_gif=True,                                  file_dir = "tests/wiener_process_shape", file_name = "test.gif", x_view=20, y_view=20, z_view=-10):
    ...
    sphere_process = compute_diffusion_process(sphere_function_x, sphere_function_y, sphere_function_z,
                                         theta = theta_shape, phi = phi_shape, Q = Q, b = b, sigma = sigma, n = n, t=t, n_step=n_step, make_gif = False)
